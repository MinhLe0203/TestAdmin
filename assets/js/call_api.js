...
function showAddClassForm(){
    ...
    // button add
    var buttonAdd = document.getElementById("add__class--confirm");
...
function showAddClassRoomForm(){
    ...
    // button add
    var buttonAdd = document.getElementById("classRoom_button--add");
    if(style.display === "none"){
...
async function fetchApiWithPageNumber(pageNumber) {
    try {
        const getAccountUrl = `https://localhost:7013/api/Class?pageIndex=${pageNumber}&pageSize=${10}`;
        const response = await fetch(getAccountUrl);
        const data = await response.json();
